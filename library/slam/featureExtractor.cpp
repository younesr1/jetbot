...
featureExtractor::featureExtractor()
    : _firstExtraction(true), _orb(cv::ORB::create()) {}

featureExtractor::featureData
featureExtractor::extractFeatures(const cv::Mat &frame) {
  ...
  // Vectors for holding all the keypoint-locators and descriptors of the good
  // features found
  std::vector<cv::KeyPoint> keypoints;
  cv::Mat descriptor;
  //! LOCATORS: Find good features to track
  cv::goodFeaturesToTrack(bwFrame, tempPoints, _maxFeatures, _rejectionRegion,
                          _minDist);
  ...
  //! MATCHER: {table_number = 12, key_size = 6, multi_probe_level = 2} TODO try
  //! also 6,12,1
  if (_firstExtraction) {
  ...
  for (auto const &i : matches) {
    ...
    if (i.at(0).distance < (_ratio * i.at(1).distance)) {
      matchedKeypoints.push_back(std::vector<cv::KeyPoint>{
          keypoints.at(i.at(0).queryIdx), _oldKeypoints.at(i.at(0).trainIdx)});
    }
  }
  //! RANSAC: Use RANSAC algorithm to filter out outliers: SKIPPED since no
  //! RANSAC for cpp
  _oldDescriptor = descriptor;
  _oldKeypoints = keypoints;
